...
We apply a version of our TCC-v method to the data of \citet{bae_why_2015} 
%(see SIX for an analogous analysis of the data of \citet{panichello_error-correcting_2019}% and for new data collected in humans on a task more similar to that deployed in monkeys by the present authors) 
and find evidence for both types of mechanism (Figure 6).
...
Visual inspection of this choice probability matrix reveals the presence of both the structure associated with stimulus-space non-uniformity and the structure associated with cognitive bias - the blue category seems to be predominantly driven by the former whereas the orange category seems to be driven predominantly by the latter.
For the blue category we see a general dilation of responses away from the diagonal (resulting in a similarity function fit by a wider Gaussian, see upper panel of Figure 6c), whereas for the orange category we see a fairly narrow Gaussian (upper panel, Figure 6c) but with nearby hues having offset responses (lower panel, Figure 6c)
to the left of the orange category the responses appear to drop below the unity diagonal and to the right they appear to rise above (highlighted in the lower panel of Figure 6c), whereas .
