...
We then fit a Gaussian with a variable floor (Equation \ref{eq:GaussianEquation}) to the error distribution for each cue. 
...
\begin{equation} \label{eq:GaussianEquation}
    \eqnmarkbox[NavyBlue]{p1}{a}
    \cdot
    \exp
    -\frac{(x-
    \eqnmarkbox[WildStrawberry]{b}{b}
    )^2}{2 
    \eqnmarkbox[OliveGreen]{c}{c}^2}
    +
    \eqnmarkbox[Plum]{d}{d}        
\end{equation}

\annotate[yshift=1em]{above,left}{p1}{height of the curve's peak}
\annotate[yshift=1em]{above}{b}{the position of the center of the peak}
\annotate[yshift=-0.75em]{below,left}{c}{the standard deviation}
