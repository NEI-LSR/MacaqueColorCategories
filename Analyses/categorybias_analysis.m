...
%% Calculate bias from similarity matrix

if ~isempty(similarityMatrix)
    
    if size(similarityMatrix,2) ~= nBig
        similarityMatrix = reshape(similarityMatrix(1:end),[nBig,nBig]); % similarity matrix  
    end
    
    %circshift similarity matrix
    sm_centered = zeros(size(similarityMatrix));
    for i = 1:nBig
        sm_centered(i,:) = circshift(similarityMatrix(i,:), (nBig/2)-i);
    end
    
    ap = 1000; % approximation, higher the value - the less of an approximation this is (but the slower it will run)
    ...
    for i = 1:nBig
        t = [];
        for j = 1:nBig
            t = [t,deg2rad(linspace(bin_starts(j),bin_ends(j),round(sm_centered(i,j)*ap)))];
        end
        bias(i) = rad2deg(circ_median(t,2));
    end
    
    ci = []; % change when confidence interval calculation is added for bias from sim matrix
